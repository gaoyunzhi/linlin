...
class WP2Static_GitHub extends WP2Static_SitePublisher
{
    public function __construct()
    {
        ...
        list($this->user, $this->repository) = explode('/', $this->settings['ghRepo']);
        $this->api_base             = 'https://api.github.com/repos/';
        $this->previous_hashes_path = $this->settings['wp_uploads_path'] . '/WP2STATIC-GITHUB-PREVIOUS-HASHES.txt';
        if (defined('WP_CLI')) {
            return;
        }
    ...
    public function upload_files()
    {
        ...
        foreach ($lines as $line) {
            ...
            $this->logAction("Uploading {$local_file} to {$this->target_path} in GitHub");
            ...
            if (isset($this->file_paths_and_hashes[$this->target_path])) {
                ...
                if ($prev != $current) {
                    ...
                    $this->recordFilePathAndHashInMemory($this->target_path, $this->local_file_contents);
                } else {
                    $this->logAction("Skipping {$this->target_path} as identical " . 'to deploy cache');
                }
            } else {
                if ($this->fileExistsInGitHub()) {
                    $this->updateFileInGitHub();
    ...
    {
        try {
            $this->remote_path = $this->api_base . $this->settings['ghRepo'] . '/contents/' . '.WP2Static/' . uniqid();
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($post_options));
    ...
    {
        $this->remote_path = $this->api_base . $this->settings['ghRepo'] . '/contents/' . $this->target_path;
        $this->query       = <<<JSON
query{
...
JSON;
        ...
        $this->client->postWithJSONPayloadCustomHeaders('https://api.github.com/graphql', $post_options, $headers, $curl_options = array(
            CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1_2
        ));
        $this->logAction("API response code {$this->client->status_code}");
        $this->logAction("API response body {$this->client->body}");
        $this->checkForValidResponses($this->client->status_code, array(
            '200',
            '201',
            '301',
            '302',
        ...
        $this->existing_file_object = $gh_file_info['data']['repository']['object'];
        $action                     = '';
        $commit_message             = '';
    ...
    public function updateFileInGitHub()
    {
        ...
        if (isset($this->settings['ghCommitMessage'])) {
            $commit_message = str_replace(array(
                '%ACTION%',
                '%FILENAME%'
            ), array(
                $action,
            ...
            $commit_message = 'WP2Static ' . $action . ' ' . $this->target_path;
        }
        try {
            $post_options = array(
    ...
    public function createFileInGitHub()
    {
        ...
        try {
            ...
            $parsed_remote_path = implode('/', array_map('rawurlencode', explode('/', $this->remote_path)));
            $parsed_remote_path = str_replace('%3A//', '://', $parsed_remote_path);
            $this->client->putWithJSONPayloadCustomHeaders($parsed_remote_path, $post_options, $headers, $curl_options = array(
                CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1_2
            ));
            $this->logAction("API response code {$this->client->status_code}");
            $this->logAction("API response body {$this->client->body}");
